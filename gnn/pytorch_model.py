...
class HeteroRGCNLayer(nn.Module):
    def __init__(self, in_size, out_size, etypes):
        ...
        # print('Finished HeteroGCN initialization')

    def forward(self, G, feat_dict):
        # print('In forward method of HeteroGCNLayer')
...
class HeteroRGCN(nn.Module):
    def __init__(self, ntype_dict, etypes, in_size, hidden_size, out_size, n_layers, embedding_size):
        ...
        embed_dict = {ntype: nn.Parameter(torch.Tensor(num_nodes, in_size))
                      # for ntype, num_nodes in ntype_dict.items() if ntype != 'review'}
                      for ntype, num_nodes in ntype_dict.items() if ntype != 'target'} # removed for yelpchi dataset
        ...
        # print('Initialized the values in the init method of HeteroGCN')

    def forward(self, g, features):
        # print('In forward method of HeteroGCN class')
        ...
        h_dict['target'] = features #is not required for yelpchi dataset
        # h_dict['review'] = features #is required for yelpchi dataset

        # print('Passing through all the layers')
        ...
        # print('Get user logits right before the return')
        # get user logits
        # return self.layers[-1](h_dict['review']) #is required for yelpchi dataset
